...
- [x] 1.2 Create `docs/game-content/census.md` presenting the same inventory readably, distinguishing stored sources from time-dependent served bytes and labeling the inactive mods pipeline explicitly; verify consistency against the JSON via the verifier cross-check

## 2. Offline verifier

- [x] 2.1 Implement `tools/content-census/verify_content.py` using standard-library structural analysis of `config/main.json`, `config/patch/patches.txt`, patch files, and `mods/mods.txt` (parse only, never import the legacy application or apply patches) with exit codes 0/1/2 for agreement/drift/invalid; verify by running it against the current sources and confirming exit 0
