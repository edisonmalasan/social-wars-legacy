import contextlib
import hashlib
import io
import json
import re
import sys
from pathlib import Path
import shutil
import tempfile
import unittest
from unittest import mock

ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "packages" / "game-content" / "tools"))

import build_offers as builder

EXPECTED_READS = frozenset([
    "config/main.json",
    "config/patch/patches.txt",
    "config/patch/atom_fusion_item.json",
    "config/patch/unit_patch.json",
    "config/patch/atom_fusion_items_data.json",
    "config/patch/atom_fusion_powerup.json",
    "config/patch/targets.json",
    "mods/mods.txt",
    "packages/game-content/schemas/offer_pack.schema.json",
    "packages/game-content/normalized/buildings.json",
    "packages/game-content/normalized/units.json",
    "packages/game-content/normalized/specials.json",
    "packages/game-content/manifest.json",
])

EXPECTED_WRITES = frozenset([
    "packages/game-content/normalized/offer_packs.json",
    "packages/game-content/manifest.json",
])


def snapshot(root):
    return {str(path.relative_to(root)).replace("\\", "/"):
            ("directory" if path.is_dir() else path.read_bytes())
            for path in Path(root).rglob("*")}


def run_main(argv):
    stdout = io.StringIO()
    stderr = io.StringIO()
    with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
        code = builder.main(argv)
    return code, stdout.getvalue(), stderr.getvalue()


def make_work_tree():
    temporary = tempfile.TemporaryDirectory()
    work = Path(temporary.name) / "repo"
    shutil.copytree(ROOT / "config", work / "config")
    shutil.copytree(ROOT / "mods", work / "mods")
    (work / "packages" / "game-content" / "tools").mkdir(parents=True)
    shutil.copy2(ROOT / "packages" / "game-content" / "tools" / "build_offers.py",
                 work / "packages" / "game-content" / "tools" / "build_offers.py")
    (work / "packages" / "game-content" / "schemas").mkdir(parents=True)
    shutil.copy2(ROOT / "packages" / "game-content" / "schemas" / "offer_pack.schema.json",
                 work / "packages" / "game-content" / "schemas" / "offer_pack.schema.json")
    (work / "packages" / "game-content" / "normalized").mkdir(parents=True)
    for name in ("buildings.json", "units.json", "specials.json"):
        shutil.copy2(ROOT / "packages" / "game-content" / "normalized" / name,
                     work / "packages" / "game-content" / "normalized" / name)
    shutil.copy2(ROOT / "packages" / "game-content" / "manifest.json",
                 work / "packages" / "game-content" / "manifest.json")
    return temporary, work


def read_work_json(work, relative):
    return json.loads((work / relative).read_text(encoding="utf-8"))


def write_work_json(work, relative, document):
    (work / relative).write_text(json.dumps(document), encoding="utf-8")


def load_work_offers(work):
    return read_work_json(work, "config/main.json")["offer_packs"]


def save_work_offers(work, entries):
    document = read_work_json(work, "config/main.json")
    document["offer_packs"] = entries
    write_work_json(work, "config/main.json", document)


class ShapeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        layers = builder.load_all(str(ROOT))
        cls.layers = layers
        by_id = {entry["id"]: entry for entry in layers["offer_packs"]}
        cls.by_id = by_id
        cls.flat = builder.coerce_offer(by_id[1], "offer id 1")
        cls.pairs = builder.coerce_offer(by_id[4], "offer id 4")
        cls.groups = builder.coerce_offer(by_id[5], "offer id 5")
        cls.null = builder.coerce_offer(by_id[18], "offer id 18")

    def test_shape_classes(self):
        self.assertEqual(self.flat["items_shape"], "flat")
        self.assertEqual(self.pairs["items_shape"], "pairs")
        self.assertEqual(self.groups["items_shape"], "groups")
        self.assertEqual(self.null["items_shape"], "null")
        self.assertIsNone(self.null["items"])

    def test_scalars_verbatim(self):
        self.assertEqual(self.flat["id"], 1)
        self.assertEqual(self.flat["name"], "starter pack")
        self.assertEqual(self.flat["cost_cash"], 25)
        for field in builder.OFFER_AMOUNTS:
            self.assertIs(type(self.flat[field]), int)
        self.assertEqual(self.null["type"], "premium_account")

    def test_flat_repetition_preserved(self):
        powerful = builder.coerce_offer(self.by_id[2], "offer id 2")
        self.assertEqual(len(powerful["items"]), 20)
        self.assertEqual(len(set(powerful["items"])), 4)

    def test_pairs_seconds_carried(self):
        self.assertEqual(self.pairs["items"],
                         [[67, 35], [85, 70], [1046, 90], [1037, 100]])

    def test_rejects_bool_amount(self):
        mutated = dict(self.by_id[1])
        mutated["cost_cash"] = True
        with self.assertRaises(builder.ValidationFailure):
            builder.coerce_offer(mutated, "mutated")

    def test_rejects_string_amount(self):
        mutated = dict(self.by_id[1])
        mutated["gold"] = "5000"
        with self.assertRaises(builder.ValidationFailure):
            builder.coerce_offer(mutated, "mutated")

    def test_rejects_negative_amount(self):
        mutated = dict(self.by_id[1])
        mutated["xp"] = -1
        with self.assertRaises(builder.ValidationFailure):
            builder.coerce_offer(mutated, "mutated")

    def test_rejects_empty_flat(self):
        mutated = dict(self.by_id[1])
        mutated["items"] = []
        with self.assertRaises(builder.ValidationFailure):
            builder.coerce_offer(mutated, "mutated")

    def test_rejects_mixed_items(self):
        mutated = dict(self.by_id[1])
        mutated["items"] = [1042, [84]]
        with self.assertRaises(builder.ValidationFailure):
            builder.coerce_offer(mutated, "mutated")


class ClassificationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        layers = builder.load_all(str(ROOT))
        cls.layers = layers
        cls.offers_section, cls.payloads = builder.build_package(
            str(ROOT), str(ROOT))

    def test_counts_and_shapes(self):
        counts = self.offers_section["counts"]
        self.assertEqual(counts["stored_offers"], 44)
        self.assertEqual(counts["offers"], 44)
        self.assertEqual(counts["shape_counts"],
                         {"null": 1, "flat": 5, "pairs": 1, "groups": 37})

    def test_refs_resolve_outside_allowlist(self):
        union = self.layers["items_id_set"]
        self.assertEqual(len(union), 900)
        offers = json.loads(self.payloads[builder.OFFERS_FILE])
        for item in offers:
            for key in item["item_refs"]:
                self.assertIn(key, union)

    def test_pinned_anomalies_recorded(self):
        anomalies = self.offers_section["anomalies"]
        self.assertEqual(len(anomalies), 2)
        offers = json.loads(self.payloads[builder.OFFERS_FILE])
        by_id = {item["id"]: item for item in offers}
        self.assertIn(35, [group[1] for group in by_id[4]["items"]])
        self.assertTrue(any(value == 1072.1224
                            for group in by_id[35]["items"]
                            for value in group))
        self.assertEqual(by_id[35]["item_refs"],
                         [str(value) for group in by_id[35]["items"]
                          for value in group
                          if isinstance(value, int) and str(value) in
                          self.layers["items_id_set"]])

    def test_no_patch_targets_offers_content(self):
        self.assertEqual(self.offers_section["inputs"]["offers_patch_targets"],
                         "none")
        self.assertEqual(self.offers_section["inputs"]["patch_order"],
                         ["atom_fusion_item", "unit_patch",
                          "atom_fusion_items_data", "atom_fusion_powerup",
                          "targets"])

    def test_envelope_fields_present(self):
        offers = json.loads(self.payloads[builder.OFFERS_FILE])
        for item in offers:
            for field in ("legacy_id", "kind", "source_file",
                          "source_layer", "content_version"):
                self.assertIn(field, item)
            self.assertEqual(item["kind"], "offer_pack")
            self.assertEqual(item["source_layer"], "stored")
            self.assertEqual(item["source_file"], "config/main.json")
            self.assertEqual(item["legacy_id"], str(item["id"]))
            self.assertEqual(item["content_version"],
                             self.offers_section["content_fingerprint"])
            self.assertIn(item["items_shape"], ("null", "flat", "pairs", "groups"))
        nulls = [item for item in offers if item["items_shape"] == "null"]
        self.assertEqual(len(nulls), 1)
        self.assertIsNone(nulls[0]["items"])
        self.assertEqual(nulls[0]["item_refs"], [])

    def test_order_matches_stored(self):
        offers = json.loads(self.payloads[builder.OFFERS_FILE])
        self.assertEqual([item["legacy_id"] for item in offers],
                         [str(entry["id"]) for entry in self.layers["offer_packs"]])


class ValidationFailureTests(unittest.TestCase):
    def setUp(self):
        self.temporary, self.work = make_work_tree()
        self.addCleanup(self.temporary.cleanup)

    def build(self):
        return run_main(["--repo-root", str(self.work),
                         "--out-root", str(self.work)])

    def test_clean_work_tree_build_exit_0(self):
        code, stdout, _ = self.build()
        self.assertEqual(code, 0)
        report = json.loads(stdout)
        self.assertEqual(report["result"], "success")
        self.assertEqual(report["counts"]["offers"], 44)

    def test_duplicate_offer_id_exit_1(self):
        entries = load_work_offers(self.work)
        entries.append(dict(entries[0]))
        save_work_offers(self.work, entries)
        code, stdout, _ = self.build()
        self.assertEqual(code, 1)
        self.assertIn("duplicate", " ".join(json.loads(stdout)["problems"]))

    def test_unresolvable_flat_reference_exit_1(self):
        entries = load_work_offers(self.work)
        entries[0]["items"] = [999999]
        save_work_offers(self.work, entries)
        code, stdout, _ = self.build()
        self.assertEqual(code, 1)
        self.assertIn("unresolvable", " ".join(json.loads(stdout)["problems"]))

    def test_non_allowlisted_group_anomaly_exit_1(self):
        entries = load_work_offers(self.work)
        for entry in entries:
            if entry["id"] == 5:
                entry["items"][0][0] = 999999
        save_work_offers(self.work, entries)
        code, stdout, _ = self.build()
        self.assertEqual(code, 1)
        self.assertIn("unresolvable", " ".join(json.loads(stdout)["problems"]))

    def test_missing_offer_field_exit_1(self):
        entries = load_work_offers(self.work)
        del entries[0]["name"]
        save_work_offers(self.work, entries)
        code, stdout, _ = self.build()
        self.assertEqual(code, 1)
        self.assertIn("missing", " ".join(json.loads(stdout)["problems"]))

    def test_missing_pinned_anomaly_exit_1(self):
        entries = load_work_offers(self.work)
        for entry in entries:
            if entry["id"] == 4:
                entry["items"] = [[67, 70], [85, 70], [1046, 90], [1037, 100]]
        save_work_offers(self.work, entries)
        code, stdout, _ = self.build()
        self.assertEqual(code, 1)
        self.assertIn("pinned anomaly", " ".join(json.loads(stdout)["problems"]))

    def test_failure_writes_nothing(self):
        before_manifest = (self.work / builder.MANIFEST_FILE).read_bytes()
        entries = load_work_offers(self.work)
        del entries[0]["name"]
        save_work_offers(self.work, entries)
        code, _, _ = self.build()
        self.assertEqual(code, 1)
        for name in EXPECTED_WRITES:
            if name == "packages/game-content/manifest.json":
                continue
            self.assertFalse((self.work / name).exists(), name)
        self.assertEqual((self.work / builder.MANIFEST_FILE).read_bytes(),
                         before_manifest)

    def test_offers_patch_target_exit_2(self):
        patch = self.work / "config" / "patch" / "targets.json"
        operations = json.loads(patch.read_text(encoding="utf-8"))
        operations.append({"op": "add", "path": "/offer_packs/-", "value": {}})
        patch.write_text(json.dumps(operations), encoding="utf-8")
        code, _, stderr = self.build()
        self.assertEqual(code, 2)
        self.assertIn("offers content", stderr)

    def test_active_mod_exit_2(self):
        mods = self.work / "mods" / "mods.txt"
        mods.write_text(mods.read_text(encoding="utf-8") + "\nno_hiring_needed\n",
                        encoding="utf-8")
        code, _, stderr = self.build()
        self.assertEqual(code, 2)
        self.assertIn("active mods", stderr)

    def test_unparseable_content_exit_2(self):
        (self.work / "config" / "main.json").write_text("{broken",
                                                        encoding="utf-8")
        code, _, stderr = self.build()
        self.assertEqual(code, 2)
        self.assertIn("not valid json", stderr)

    def test_missing_items_edge_exit_2(self):
        (self.work / "packages" / "game-content" / "normalized"
         / "units.json").unlink()
        code, _, stderr = self.build()
        self.assertEqual(code, 2)
        self.assertIn("normalized items", stderr)


class TraceabilityTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        _offers_section, payloads = builder.build_package(str(ROOT), str(ROOT))
        cls.offers_schema = json.loads(
            (ROOT / builder.OFFERS_SCHEMA_FILE).read_text(encoding="utf-8"))
        cls.offer = json.loads(payloads[builder.OFFERS_FILE])[0]

    def test_required_is_subset_of_properties(self):
        for field in self.offers_schema["required"]:
            self.assertIn(field, self.offers_schema["properties"],
                          "required not in properties: " + field)

    def test_every_required_field_is_enforced(self):
        for field in self.offers_schema["required"]:
            mutated = dict(self.offer)
            del mutated[field]
            problems = builder.validate_against_schema(
                mutated, self.offers_schema,
                "offer_pack " + self.offer["legacy_id"])
            self.assertTrue(
                any("missing required field" in problem
                    and field in problem for problem in problems),
                "no validator check for required field offer_pack.%s" % (field,))

    def test_wrong_types_are_rejected(self):
        cases = [("id", "1"), ("name", 5), ("type", 7), ("cost_cash", "25"),
                 ("gold", "1"), ("enabled", "1"), ("position", "6"),
                 ("items_shape", "bundle"), ("items", 1042),
                 ("item_refs", "1042")]
        for field, bad in cases:
            mutated = dict(self.offer)
            mutated[field] = bad
            problems = builder.validate_against_schema(
                mutated, self.offers_schema,
                "offer_pack " + self.offer["legacy_id"])
            self.assertTrue(problems,
                            "no type check for offer_pack.%s" % (field,))

    def test_minimum_gates_enforced(self):
        mutated = dict(self.offer)
        mutated["cost_cash"] = -1
        self.assertTrue(builder.validate_against_schema(
            mutated, self.offers_schema, "offer_pack"))
        mutated = dict(self.offer)
        mutated["items_shape"] = "bundle"
        problems = builder.validate_against_schema(
            mutated, self.offers_schema, "offer_pack")
        self.assertTrue(any("enum mismatch" in problem for problem in problems))

    def test_kind_const_and_no_extra_properties(self):
        mutated = dict(self.offer)
        mutated["kind"] = "building"
        self.assertTrue(builder.validate_against_schema(
            mutated, self.offers_schema, "offer_pack"))
        mutated = dict(self.offer)
        mutated["invented_field"] = 1
        problems = builder.validate_against_schema(
            mutated, self.offers_schema, "offer_pack")
        self.assertTrue(any("additional property" in problem
                            for problem in problems))


class RoundTripTests(unittest.TestCase):
    def test_round_trip_clean_on_real_content(self):
        layers = builder.load_all(str(ROOT))
        _offers_section, payloads = builder.build_package(str(ROOT), str(ROOT))
        offers = json.loads(payloads[builder.OFFERS_FILE])
        problems = builder.check_round_trip(offers, layers["offer_packs"])
        self.assertEqual(problems, [])

    def test_repeated_work_tree_builds_byte_identical(self):
        temporary, work = make_work_tree()
        self.addCleanup(temporary.cleanup)
        code, _, _ = run_main(["--repo-root", str(work),
                               "--out-root", str(work)])
        self.assertEqual(code, 0)
        first = {name: (work / name).read_bytes() for name in EXPECTED_WRITES}
        code, _, _ = run_main(["--repo-root", str(work),
                               "--out-root", str(work)])
        self.assertEqual(code, 0)
        for name, payload in first.items():
            self.assertEqual((work / name).read_bytes(), payload, name)


class ManifestTests(unittest.TestCase):
    def test_manifest_matches_produced_output(self):
        temporary, work = make_work_tree()
        try:
            code, _, _ = run_main(["--repo-root", str(work),
                                   "--out-root", str(work)])
            self.assertEqual(code, 0)
            manifest = read_work_json(work, "packages/game-content/manifest.json")
            self.assertIn("offers", manifest)
            offers_section = manifest["offers"]
            self.assertEqual(offers_section["result"], "success")
            self.assertEqual(offers_section["policy"], builder.POLICY)
            self.assertEqual(offers_section["coercion_ruleset"],
                             builder.COERCION_RULESET_VERSION)
            counts = offers_section["counts"]
            offers = read_work_json(
                work, "packages/game-content/normalized/offer_packs.json")
            self.assertEqual(len(offers), counts["offers"])
            self.assertEqual(counts["stored_offers"], 44)
            self.assertEqual(counts["shape_counts"],
                             {"null": 1, "flat": 5, "pairs": 1, "groups": 37})
            self.assertEqual(len(offers_section["anomalies"]), 2)
            self.assertEqual(offers_section["content_fingerprint"],
                             builder.fingerprint_inputs(
                                 builder.load_all(str(work))))
            self.assertEqual(offers_section["inputs"]["items_reference_edge"]["union_legacy_ids"],
                             900)
            by_file = {entry["file"]: entry
                       for entry in offers_section["outputs"]}
            self.assertEqual(set(by_file),
                             set(EXPECTED_WRITES)
                             - {"packages/game-content/manifest.json"})
            data = (work / "packages/game-content/normalized/offer_packs.json").read_bytes()
            self.assertEqual(by_file["packages/game-content/normalized/offer_packs.json"]["bytes"],
                             len(data))
            self.assertEqual(by_file["packages/game-content/normalized/offer_packs.json"]["sha256"],
                             hashlib.sha256(data).hexdigest())
            self.assertTrue(offers_section["notes"])
            # Prior keys survive the offers merge untouched.
            self.assertEqual(manifest["policy"], "items-normalization-v1")
            self.assertEqual(manifest["counts"]["loaded_items"], 900)
            self.assertEqual(manifest["quests"]["policy"],
                             "quest-normalization-v1")
            self.assertEqual(manifest["tables"]["policy"],
                             "tables-normalization-v1")
            self.assertEqual(manifest["economy"]["policy"],
                             "economy-normalization-v1")
            self.assertEqual(manifest["social"]["policy"],
                             "social-normalization-v1")
            self.assertEqual(manifest["taxonomy"]["policy"],
                             "taxonomy-normalization-v1")
            self.assertEqual(manifest["darts"]["policy"],
                             "darts-normalization-v1")
            self.assertEqual(manifest["globals"]["policy"],
                             "globals-normalization-v1")
            self.assertEqual(manifest["globals"]["counts"]["globals"], 105)
        finally:
            temporary.cleanup()


class ContainmentTests(unittest.TestCase):
    def guarded_run(self, work, argv):
        opened = []
        real_io_open = io.open
        real_builtins_open = open

        def guarded_open(file, mode="r", *args, **kwargs):
            if any(flag in str(mode) for flag in ("w", "a", "x", "+")):
                opened.append(("write", str(file)))
                return (real_builtins_open if guarded_open.is_builtin
                        else real_io_open)(file, mode, *args, **kwargs)
            opened.append(("read", str(file)))
            return (real_builtins_open if guarded_open.is_builtin
                    else real_io_open)(file, mode, *args, **kwargs)

        def io_guard(file, mode="r", *args, **kwargs):
            guarded_open.is_builtin = False
            return guarded_open(file, mode, *args, **kwargs)

        def builtins_guard(file, mode="r", *args, **kwargs):
            guarded_open.is_builtin = True
            return guarded_open(file, mode, *args, **kwargs)

        before = snapshot(work)
        # pathlib.Path.read_bytes resolves to io.open, not builtins.open,
        # so both entry points are guarded.
        with mock.patch("io.open", io_guard), \
                mock.patch("builtins.open", builtins_guard):
            code, stdout, stderr = run_main(argv)
        return code, stdout, stderr, opened, before

    def observed_sets(self, work, opened):
        reads = set()
        writes = set()
        for kind, raw in opened:
            try:
                relative = str(Path(raw).relative_to(work)).replace("\\", "/")
            except ValueError:
                self.fail("tool touched outside repo root: %r" % (raw,))
            if kind == "read":
                reads.add(relative)
            else:
                writes.add(relative)
        return reads, writes

    def test_success_reads_only_expected_and_inputs_unchanged(self):
        temporary, work = make_work_tree()
        try:
            code, _, _, opened, before = self.guarded_run(
                work, ["--repo-root", str(work), "--out-root", str(work)])
            self.assertEqual(code, 0)
            reads, writes = self.observed_sets(work, opened)
            self.assertEqual(reads, EXPECTED_READS)
            self.assertEqual(writes, EXPECTED_WRITES)
            after = snapshot(work)
            new_files = {path for path in set(after) - set(before)
                         if after[path] != "directory"}
            self.assertEqual(new_files,
                             {"packages/game-content/normalized/offer_packs.json"})
            for path, payload in before.items():
                if path == "packages/game-content/manifest.json":
                    continue
                self.assertEqual(after[path], payload, path)
        finally:
            temporary.cleanup()

    def test_failure_writes_nothing_and_inputs_unchanged(self):
        temporary, work = make_work_tree()
        try:
            entries = load_work_offers(work)
            del entries[0]["name"]
            save_work_offers(work, entries)
            code, _, _, opened, before = self.guarded_run(
                work, ["--repo-root", str(work), "--out-root", str(work)])
            self.assertEqual(code, 1)
            _reads, writes = self.observed_sets(work, opened)
            self.assertEqual(writes, set())
            self.assertEqual(snapshot(work), before)
        finally:
            temporary.cleanup()

    def test_no_legacy_or_network_imports(self):
        source = (ROOT / "packages" / "game-content" / "tools"
                  / "build_offers.py").read_text(encoding="utf-8")
        for line in source.splitlines():
            match = re.match(r"\s*(import|from)\s+(\S+)", line)
            if not match:
                continue
            module = match.group(2).split(".")[0]
            self.assertNotIn(module, ("get_game_config", "jsonpatch", "flask",
                                      "server", "command", "engine",
                                      "requests", "subprocess", "socket",
                                      "urllib", "webbrowser", "http"),
                             "forbidden import: " + line)


if __name__ == "__main__":
    unittest.main()
