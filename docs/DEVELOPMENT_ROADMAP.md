...
- **Roadmap cursor:** M4 / first unit conversion
- **Active OpenSpec change:** `first-unit-conversion`
- **Lifecycle stage:** VERIFYING
- **Change status:** Proposal merged via PR #106 (`c9feb1c`): one assembled unit package with per-sprite animation inventory, referenced-fill-only validation, and a neutral shared manifest envelope. Apply implemented on `feat/first-unit-conversion` (tasks 1.1–3.1): `convert_unit.py` (depth-first `PlaceObject2` decode, `RemoveObject2`/`FrameLabel`/`ShowFrame` inventory, `SymbolClass` id-name pairs, `main` root entry, shared-parser shape records, content-ref uniqueness for unit 933, referenced-fill resolution with verbatim unreferenced `65535` placeholders), `unit_package.schema.json`, shared fill-array byte-alignment + nested schema recursion + `conversion-v1` envelope migration in `convert_building.py`, regenerated `conversions.json` (two per-entry-policy entries) and `statuses.json` (both sources converted), the 48-test suite `test_convert_unit.py`, README/`docs/assets/registry.md`/AGENTS.md documentation. Independent verification pass completed: no CRITICAL findings; all 16 spec scenarios mapped (15 with direct tests, house byte-identity measured); warnings repaired — W1 extraction-prerequisite doc accuracy (tracked `statuses.json` downgrade + LF manifest form now documented in README/design), W2 added the house byte-identity regression test (fresh package bytes/digests vs committed), O1 scoped the legacy `content_version` drop to the legacy migration branch with a verbatim-preservation test. Directly executed checks under pinned Windows x64 CPython 3.9.13: building suite 18 OK, unit suite 48 OK, full asset-registry suite 147 OK, both converters exit 0 with `conversions.json` sha256 `c6fc2ad31463…` and `statuses.json` `078b8dfb5a7b…` unchanged by the repairs, house package byte-identical to HEAD (`eab51e269f419cb9a4951bb7c7972acf3b960e99`), change strict validation PASS, specs strict validation 25/25 PASS, `git diff --check` clean; Apply PR pending.
- **Current objective:** verification passed and the final diff reviewed; open the Apply PR (merge commit only), then spec sync and archive for this change.
- **Last completed change:** `first-building-conversion` — archived as `2026-09-22-first-building-conversion`; proposal PR #102, Apply PR #103, sync PR #104
- **Next eligible objective:** after `first-unit-conversion` sync + archive: the next M4 slice per roadmap §14 prioritization (terrain / essential HUD) or an M4 exit assessment; no Godot project inside M4.
...
- **Last OpenSpec validation:** PASS — `first-unit-conversion` change strict validation and `openspec validate --specs --strict` 25/25 PASS (2026-09-26); this is not the unavailable verification workflow
...
- **Last verified commit:** `c9feb1c` (Merge pull request #106, first-unit-conversion proposal)
