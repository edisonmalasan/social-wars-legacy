...
- **Roadmap cursor:** M3 / offers normalization
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `normalize-offers` fully complete. Proposal merged via PR #77 (`f23112f`): 44 offer packs with shape classes, verbatim preservation, fail-closed refs with exact-match anomaly allowlist. Apply merged via PR #78 (`8b0c554`): implementation `77778f6` (builder/validator, offer_pack schema, normalized output, manifest `offers` section, README + AGENTS.md docs, 37-test suite). Spec sync merged via PR #79 (`610993e`): new main spec `openspec/specs/offers-normalization/spec.md`. Change archived as `2026-09-22-normalize-offers`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 37 offers tests OK, 384 total game-content tests OK, change + specs strict validations PASS (19/19); staged diff reviewed (8 files, +4121 insertions, no legacy sources touched) and `git diff --check` clean.
- **Current objective:** Propose the next smallest coherent M3 change: the remaining `images` asset index (607 entries, M4-adjacent — normalize as a validated path registry or explicitly defer to the M4 asset pipeline with classification), the canonical domain model (roadmap section 8), or the Compatibility API v0 groundwork — whichever bounds smaller against the normalized package precedent.
- **Last completed change:** `normalize-offers` — archived as `2026-09-22-normalize-offers`; proposal PR #77, Apply PR #78, sync PR #79
- **Next eligible objective:** `images` disposition (normalize vs defer-to-M4), domain model, or Compatibility API v0 groundwork after this archive; authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: offer shapes null 1 / flat 5 / pairs 1 / groups 37; pair seconds opaque numbers; pinned anomalies (offer 4, 35) and (offer 35, 1072.1224 with siblings carrying 1072/1224) preserved verbatim under an exact-match allowlist that fails on any other unresolving leaf. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 19/19 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Offers acceptance complete (proposal PR #77, Apply PR #78, sync PR #79, archive in progress). Directly executed: build exit 0 (44 offers), 37 tests OK, 384 total game-content tests OK, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `610993e` (Merge pull request #79, offers spec sync)
