...
- **Roadmap cursor:** M4 / first building conversion
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `first-building-conversion` fully complete. Proposal merged via PR #102 (`cc55b6d`): single-building package assembly with shape-style parsing, no tessellation, no rendering. Apply merged via PR #103 (`bba4e02`): implementation `a2e86d4` (shape-style parser with edge counts, package assembly with digest-equal bitmap copies, 2 schemas, package + conversions.json + statuses merge, README + docs + AGENTS.md docs, 17-test suite). Spec sync merged via PR #104 (`63213a2`): new main spec `openspec/specs/first-building-conversion/spec.md`. Change archived as `2026-09-22-first-building-conversion`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 17 converter tests OK, 98 asset-registry tests OK, 412 M3 tests intact, change + specs strict validations PASS (25/25); staged diff reviewed (14 files, +1627 insertions, no sources modified, no rendering, no Godot project) and `git diff --check` clean.
- **Current objective:** M4 slices 1–5 done (registry, inspection, sounds, bitmaps, first building). Next smallest coherent M4 objective: first-unit conversion (one authentic unit package with multi-frame animation inventory, verified by inspection without Godot) — the second half of the M4 exit. Any Godot rendering verification stays later (M5/M6 own Godot).
...
- **Next eligible objective:** SWF static inspection, then conversion tooling and converted building/unit (no Godot project inside M4); authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: house shape bounds 216×144 px with one `0x41` bitmap fill to id 1, 4 straight edges, MoveTo deltas use dedicated MoveBits counts (fill+line sum is a known pitfall), NewStyles blocks parse into style arrays (no tessellation anywhere); package bitmaps digest-equal to extraction outputs; matrices raw, scripts uninterpreted, no Godot project created. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 25/25 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Building-conversion acceptance complete (proposal PR #102, Apply PR #103, sync PR #104, archive in progress). Directly executed: build exit 0 (1 package), 17 tests OK, 98 asset-registry tests OK, 412 M3 tests intact, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `63213a2` (Merge pull request #104, building conversion spec sync)
