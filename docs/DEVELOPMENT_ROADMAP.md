...
- **Current objective:** verifier re-check PASS — open the Apply PR into `main` (merge commit), then Sync (delta → main spec) and Archive for `first-render-in-godot`. Prior context: proposal PR #111 (`e745466`); `first-unit-conversion` archived via PR #109; M4 deliverables complete; Godot 4.7.2 installed and pinned (docs PR #110, `615c739`).
...
- **Next eligible objective:** close the Apply stage for `first-render-in-godot`, then Sync and Archive; after archiving, the next roadmap decision is M5 Godot Foundation (project exists now but GameApi/compatibility boot remain) or the §14 content slice.
