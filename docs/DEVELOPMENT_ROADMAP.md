...
- **Current milestone:** M5 — Godot Foundation
- **Roadmap cursor:** M5 / bootstrap vertical slice (roadmap §16 GameApi abstraction + §17 Compatibility API v0 + §18 bootstrap loading)
- **Active OpenSpec change:** `godot-compatibility-boot` — M5 smallest coherent objective: read-only boot slice (Compatibility API v0 adapting the unchanged legacy boot modules, `GameApi` autoload with `LegacyV0Api`/`FakeApi`, boot scene as main scene, executed-legacy fixtures as the parity oracle), with `first-render-in-godot` requirement R1 MODIFIED to the M5 foundation allow-list boundary
- **Lifecycle stage:** IMPLEMENTING
- **Change status:** Implementing on `feat/godot-compatibility-boot` — proposal stage merged via PR #115 (`63ce384`, merge commit); implementation proceeds in two sequential units with clean file boundaries (unit A: executed-legacy fixture capture + `apps/compat-api/` service and tests; unit B: `GameApi`/boot scene/`verify-boot.ps1`/docs), then an independent verification pass. All four artifacts written at propose time (`proposal.md`, `design.md` with decisions D1–D10, `tasks.md` 5 groups/19 tasks, capability delta `godot-compatibility-boot` ADDED with 6 requirements/16 scenarios plus `first-render-in-godot` R1 MODIFIED with 2 scenarios), strict validation PASS (2026-09-27). Prior change `first-render-in-godot`: Implemented on `feat/first-render-in-godot` (commits `0dd4bb8` Godot cache ignore, `569d2bb` loader/renderer, `8dd1eac` comparison/tests/verify command, `4a4e7cc` README, `397d12c` evidence, `5b41221` asset-registry `apps/` exclusion fix, `b546a9d` AGENTS commands + tasks, `206a451` verifier repairs). Delivered: minimal render-verification project at `apps/client-godot/` (20 tracked files, no game systems), read-only conversion-v1 loader with fail-closed envelopes, frame-1 placement walk with `fill_refs`/placeholder fail-close, SWF fill-matrix decode with bounds↔bitmap oracle (house 4320×2880 twips → 216×144 px, error 0.0), straight-alpha JPEG+PNG compositing, windowed capture → independent reference compare → `report.json`, headless comparator self-test, single command `powershell -File apps/client-godot/verify.ps1` with SHA-256 pre/post guards, committed golden evidence. Independent verifier round 1 (read-only, artifacts vs implementation): 17/18 scenarios PASS (scenario 18, the exit-assessment record, was the F1 failure), all checks re-run green (verify.ps1 exit 0 with 55 checks, asset-registry suite 147 OK under pinned CPython 3.9.13, strict change validation PASS, `git diff --check` clean, evidence byte-identical across three capture runs); findings F1 CRITICAL (this ledger entry/task 5.5 pending), F2 WARNING (`max_channel_abs` documented as AND-ed but not enforced by the comparator), observations O1–O4. Repairs: `206a451` adds an independent max-channel failure gate (self-test now records `max_abs.r/g/b` violations, five failures total), tightens the README cross-check claim to compositing (placement is cross-checked by loader tests + oracle + report assertions), and aligns the spec's `fill_refs` sentence with building envelopes that have no such field; post-repair re-runs: verify.ps1 exit 0, loader/scene/scope suites 123/36/291 exit 0, self-test exit 1 (5 failures), compare exit 0, strict validation PASS. Final verifier re-check of the repairs: PASS — F1/F2 both RESOLVED, every command re-executed green (verify.ps1 exit 0 with 55 checks confirmed by root, strict validation PASS, suites 123/36/291, self-test exit 1 with 5 failures, 147 registry tests OK), evidence byte-identical, ledger claims matched against `report.json`; re-check label observations (check/scenario counts, `0dd4bb8` description) corrected in this entry. Apply merged via PR #112 (`daaa40c`, merge commit). Sync stage: main spec `openspec/specs/first-render-in-godot/spec.md` created from the change delta (7 requirements, 18 scenarios; only the `# Spec Delta`/`## ADDED Requirements` headers transformed), `openspec validate first-render-in-godot --specs --strict` 27/27 specs PASS. Sync merged via PR #113 (`4af07a5`); `openspec archive first-render-in-godot` then moved the change to `openspec/changes/archive/2026-09-26-first-render-in-godot/` with the CLI confirming main specs already in sync (+0/~0/−0) and strict validation again 27/27.
...
- **Current objective:** Apply stage on `feat/godot-compatibility-boot` — implement tasks 1–5 of the approved change, run the full verification battery, then independent verification; PR into `main` follows. Prior context: proposal PR #115 `63ce384`; `first-render-in-godot` lifecycle closed (archive PR #114 `9208e2b`, sync PR #113 `4af07a5`, Apply PR #112 `daaa40c`, proposal PR #111 `e745466`); Godot 4.7.2 installed and pinned (docs PR #110, `615c739`).
- **Last completed change:** `first-render-in-godot` — archived as `2026-09-26-first-render-in-godot`; proposal PR #111, Apply PR #112, sync PR #113, archive PR #114. Previous: `first-unit-conversion` — archive PR #109.
- **Next eligible objective:** the objective after this change — continue M5 with ContentRegistry (§19) and the asset ID registry (§20), then GameClock/camera/basic UI foundation, until the M5 exit criterion "Client boots and communicates with Compatibility API" is assessed against the delivered slice; the §14 content slice remains an alternative. Do not skip to M6.
- **Blocking issues:** None. Source-truth observations recorded: elephant 550×400 @ 30 fps, 7 sprites, sprite 63 labels at frames 1/6/11/16/21, 28 shapes (25 with an unreferenced `65535` placeholder), 28 referenced bitmap ids = 28 JPEG3 outputs; byte-identical converter reruns additionally require the documented worktree line-ending forms of the `content_version` fingerprint inputs (README worktree-form note). Verifier observations O1–O4 accepted as non-blocking: perturbation is applied to the reference side (comparator is symmetric); reference and capture share placement resolution (cross-checked by loader tests, bounds oracle, and report assertions); building envelopes have no `fill_refs` field (placeholder fail-close covers them); the scope test scans `.gd`/`.tscn` while README/`verify.ps1` were grep-checked separately. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; no dedicated OpenSpec verification workflow is installed — strict validation, independent verification, required checks, and final diff review are used as the fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — `godot-compatibility-boot` change strict validation (2026-09-27); previously `first-render-in-godot` change strict validation plus main-specs strict validation 27/27 (2026-09-26, sync and archive stages), following `first-unit-conversion` change/specs/archive strict validation (2026-09-26, specs 26/26); this is not the unavailable verification workflow
- **Last implementation verification:** Independent read-only verifier pass over `first-render-in-godot` (2026-09-26): 18-scenario matrix → 17 PASS / 1 FAIL before repairs (F1 = scenario 18); executed checks all green (verify.ps1 exit 0 with 55 checks, loader/scene/scope suites 123/36/291, comparator self-test non-zero headless, 147 asset-registry tests under pinned CPython 3.9.13, strict change validation, `git diff --check`, preservation/forbidden-content/visual/scope clean). Findings F1 (ledger pending) and F2 (`max_channel_abs` gate unenforced) repaired in `206a451`/the ledger entry and re-run green; O1–O4 recorded under Blocking issues as accepted non-blocking observations. Final re-check of the repairs PASS (F1/F2 resolved, all commands re-run green). (Prior change: `first-unit-conversion` verifier round 2 PASS, W1/W2/O1 repaired.)
- **Last verified commit:** `9208e2b` (Merge pull request #114, first-render-in-godot archive)
