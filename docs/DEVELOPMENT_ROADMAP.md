...
- **Roadmap cursor:** M4 / bitmap extraction
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `bitmap-extraction` fully complete. Proposal merged via PR #98 (`d061c8f`): format-faithful JPEG/lossless extraction with hand-rolled PNG encoding, ignored bulk outputs. Apply merged via PR #99 (`73bb997`): implementation `7b0a46a` (extractor, 2 schemas, 36 MB digest manifest, neutral statuses envelope, ignored bulk outputs, README + docs + AGENTS.md docs, 21-test suite). Spec sync merged via PR #100 (`d7cd476`): new main spec `openspec/specs/bitmap-extraction/spec.md`. Change archived as `2026-09-22-bitmap-extraction`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 21 bitmap tests OK, 81 asset-registry tests OK, 412 M3 tests intact, change + specs strict validations PASS (24/24); staged diff reviewed (13 files, +1221803 insertions, no sources modified, no rendering) and `git diff --check` clean.
- **Current objective:** M4 slices 1–4 done (registry, inspection, sounds, bitmaps). Next smallest coherent M4 objective: first-conversion assembly (one authentic building package from extracted bitmaps plus placement metadata, verified by inspection without Godot) — then the unit counterpart. Any Godot rendering verification stays later (M5/M6 own Godot).
- **Last completed change:** `bitmap-extraction` — archived as `2026-09-22-bitmap-extraction`; proposal PR #98, Apply PR #99, sync PR #100
...
- **Blocking issues:** None for the archived change. Source-truth observations recorded: 61,702 bitmap payloads convert (46,425 JPEG3 with dimension-matched alphas, 14,364 lossless ARGB/colormap, 913 plain JPEG, zero table splices needed); colormap layout is U8 count prefix outside zlib with palette plus stride-padded rows inside; lossless channel order stored as ARGB and mapped without un-premultiplying; bulk outputs regenerate under ignored `assets/converted/images/` while the committed digest manifest is permanent. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 24/24 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Bitmap-extraction acceptance complete (proposal PR #98, Apply PR #99, sync PR #100, archive in progress). Directly executed: build exit 0 (61,702 bitmaps), 21 tests OK, 81 asset-registry tests OK, 412 M3 tests intact, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `d7cd476` (Merge pull request #100, bitmap extraction spec sync)
