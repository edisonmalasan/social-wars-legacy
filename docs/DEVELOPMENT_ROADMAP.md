...
- **Roadmap cursor:** M4 / SWF static inspection
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `swf-static-inspection` fully complete. Proposal merged via PR #90 (`efa4fc2`): static stdlib-only header/tag/symbol inventory with no execution. Apply merged via PR #91 (`9c3899e`): implementation `b4a432d` (inspector with nested-sprite recursion, 2 schemas, inspection.json for all 1,176 SWFs, README + docs + AGENTS.md docs, 21-test suite). Spec sync merged via PR #92 (`9f50db2`): new main spec `openspec/specs/swf-static-inspection/spec.md`. Change archived as `2026-09-22-swf-static-inspection`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 21 inspection tests OK, 42 asset-registry tests OK, 412 M3 tests intact, change + specs strict validations PASS (22/22); staged diff reviewed (9 files, +171480 insertions, no assets modified, no Flash execution) and `git diff --check` clean.
- **Current objective:** M4 slices 1–2 done (observe/record, parse/inventory). Next smallest coherent M4 objective: embedded-asset extraction (bitmaps/sounds out of SWF tag payloads into converted/ directories, raw sources untouched) or first-conversion tooling — whichever bounds smaller against the inspection precedent. Converted building/unit and any Godot rendering verification stay later (M5/M6 own Godot).
- **Last completed change:** `swf-static-inspection` — archived as `2026-09-22-swf-static-inspection`; proposal PR #90, Apply PR #91, sync PR #92
...
- **Blocking issues:** None for the archived change. Source-truth observations recorded: all 1,176 SWFs parse cleanly to End (CWS 10/11/15/17); nested sprite timelines merged (105,038 nested tags, max depth 1); 1,175 files carry ABC and 0 carry legacy actions, so conversion tooling must assume scripted timelines by default; declared header lengths are recorded, never trusted; LEB128 scene integers decoded (fixed-width decoding is a known pitfall). Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 22/22 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** SWF-inspection acceptance complete (proposal PR #90, Apply PR #91, sync PR #92, archive in progress). Directly executed: build exit 0 (1,176 files), 21 tests OK, 42 asset-registry tests OK, 412 M3 tests intact, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `9f50db2` (Merge pull request #92, SWF inspection spec sync)
