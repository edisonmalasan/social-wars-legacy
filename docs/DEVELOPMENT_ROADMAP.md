...
- **Change status:** `first-unit-conversion` fully complete. Proposal merged via PR #106 (`c9feb1c`): one assembled unit package with per-sprite animation inventory, referenced-fill-only validation, and a neutral shared manifest envelope. Apply merged via PR #107 (`b9a6fa9`): implementation `6de4604` (`convert_unit.py` with depth-first `PlaceObject2` decode, labels/frame inventory, `SymbolClass` pairs and `main` root entry; shared fill-array byte-alignment, `fill_refs` collection, nested schema recursion, and `conversion-v1` envelope migration in `convert_building.py`; `unit_package.schema.json`; elephant package 384701 bytes with 56 digest-equal bitmap copies; regenerated `conversions.json`/`statuses.json`; 48-test suite plus the house byte-identity regression test) and docs `a285f36` (README/`docs/assets/registry.md`/AGENTS.md sections, tasks 1.1–3.1 checked, design extract/restore/convert ordering, status ledger). Spec sync merged via PR #108 (`4104c61`): new main spec `openspec/specs/first-unit-conversion/spec.md` (7 requirements, 16 scenarios; delta and main requirement/scenario headers compared identical). Change archived as `2026-09-26-first-unit-conversion`. Independent verifier pass: no CRITICAL findings; warnings W1 (extraction-prerequisite doc accuracy), W2 (house byte-identity regression test), and O1 (`content_version` scoped to the legacy migration branch) repaired and re-verified PASS. Directly executed checks under pinned Windows x64 CPython 3.9.13: building suite 18 OK, unit suite 48 OK, full asset-registry suite 147 OK, both converters exit 0 in either order (`conversions.json` sha256 `c6fc2ad31463…`, `statuses.json` `078b8dfb5a7b…`), house package byte-identical to HEAD (`eab51e269f419cb9a4951bb7c7972acf3b960e99`), change strict validation PASS while active plus specs strict validation 26/26 PASS after sync, `git diff --check` clean.
- **Current objective:** `first-unit-conversion` fully archived via PR #109 (`12d3107`); M4 deliverables (asset registry, SWF extraction workflow, conversion tooling, one converted building, one converted unit) are complete. Godot engine prerequisite completed on this branch: Godot 4.7.2.stable installed via winget (verified `4.7.2.stable.official.ed1daf0bf`) and the exact version pinned in `AGENTS.md` per the pre-M5 requirement; no Godot project created and no OpenSpec change opened for this setup.
- **Last completed change:** `first-unit-conversion` — archived as `2026-09-26-first-unit-conversion`; proposal PR #106, Apply PR #107, sync PR #108
- **Next eligible objective:** M4 exit assessment — the exit criterion (authentic building and unit render correctly in Godot) still requires creating the Godot project this roadmap defers to M5/M6, so it needs an explicit request; the local engine prerequisite (Godot 4.7.2.stable) is now installed and pinned, so that gate is no longer a tooling blocker. Alternatively the next content slice per roadmap §14 prioritization (terrain / essential HUD). Do not begin a new change without an orchestration request.
- **Blocking issues:** None. Source-truth observations recorded: elephant 550×400 @ 30 fps, 7 sprites, sprite 63 labels at frames 1/6/11/16/21, 28 shapes (25 with an unreferenced `65535` placeholder), 28 referenced bitmap ids = 28 JPEG3 outputs; byte-identical converter reruns additionally require the documented worktree line-ending forms of the `content_version` fingerprint inputs (README worktree-form note). Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; no dedicated OpenSpec verification workflow is installed — strict validation, independent verification, required checks, and final diff review are used as the fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — `first-unit-conversion` change strict validation while active plus `openspec validate --specs --strict` 26/26 PASS after sync and `openspec validate first-unit-conversion --strict` on the archived state (2026-09-26); this is not the unavailable verification workflow
- **Last implementation verification:** Independent verifier pass completed with no CRITICAL findings (2026-09-26); warnings W1 (extraction-prerequisite doc accuracy), W2 (house byte-identity regression test), and O1 (`content_version` scoped to legacy migration) repaired and re-executed: 18 building tests, 48 unit tests, 147 full asset-registry tests OK under pinned CPython 3.9.13; house byte-identity, real-corpus order-independence (both orders → identical manifest/status/package hashes), legacy fail-closed paths, and containment verified.
