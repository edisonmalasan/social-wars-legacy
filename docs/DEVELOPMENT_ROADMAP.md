...
- **Roadmap cursor:** M3 / inventory taxonomy normalization
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `normalize-inventory-taxonomy` fully complete. Proposal merged via PR #65 (`8a12baa`): 90 inventory items / 6 categories / 20 unit-collection groups, object-key preservation pattern, two validated items-set edges, one documented opaque-code gap. Apply merged via PR #66 (`9d8a88c`): implementation `f3833ea` (builder/validator, 3 schemas, 3 normalized outputs, manifest `taxonomy` section, README + AGENTS.md docs, 47-test suite). Spec sync merged via PR #67 (`5755777`): new main spec `openspec/specs/inventory-taxonomy-normalization/spec.md`. Change archived as `2026-09-22-normalize-inventory-taxonomy`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 47 taxonomy tests OK, 279 total game-content tests OK, change + specs strict validations PASS (16/16); staged diff reviewed (12 files, +4456 insertions, no legacy sources touched) and `git diff --check` clean.
- **Current objective:** Propose the next smallest coherent M3 change: remaining content domains (offer packs with heterogeneous/nested shapes plus a stored float artifact, globals tuning constants, images asset index, darts dynamics with patch replacement), the canonical domain model (roadmap section 8), or the Compatibility API v0 groundwork — whichever bounds smaller against the normalized package precedent.
- **Last completed change:** `normalize-inventory-taxonomy` — archived as `2026-09-22-normalize-inventory-taxonomy`; proposal PR #65, Apply PR #66, sync PR #67
- **Next eligible objective:** Remaining content domains (offers, globals, images, darts), domain model, or Compatibility API v0 groundwork after this archive; authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: collection `costs` null only in key `1` and `category_name_el` empty in all 20 entries, both preserved with notes; stored item category codes 6/7/8/9 and subcategory codes 6/7/81/91 (patch-era units) have no category entry and are recorded as opaque codes, never references; stored `inventory_ids` split 775 null / 3 resolving objects. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 16/16 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Inventory-taxonomy acceptance complete (proposal PR #65, Apply PR #66, sync PR #67, archive in progress). Directly executed: build exit 0 (90/6/20), 47 tests OK, 279 total game-content tests OK, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `5755777` (Merge pull request #67, inventory taxonomy spec sync)
