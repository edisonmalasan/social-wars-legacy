...
- **Current objective:** archive stage open on `chore/archive-first-unit-conversion`; after its merge return to updated main. M4 deliverables (asset registry, SWF extraction workflow, conversion tooling, one converted building, one converted unit) are now complete.
...
- **Next eligible objective:** M4 exit assessment — the exit criterion (authentic building and unit render correctly in Godot) requires the Godot project this roadmap defers to M5/M6, so it needs an explicit request; alternatively the next content slice per roadmap §14 prioritization (terrain / essential HUD). Do not begin a new change without an orchestration request.
