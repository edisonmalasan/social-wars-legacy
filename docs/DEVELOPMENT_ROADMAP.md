...
- **Roadmap cursor:** M3 / social tables normalization
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `normalize-social-tables` fully complete. Proposal merged via PR #61 (`a78a86f`): 5 neighbor assists / 10 findable items / 26 social items, verbatim native amounts and display strings, no cross-references, no patch layering. Apply merged via PR #62 (`c03dbc0`): implementation `faee660` (builder/validator, 3 schemas, 3 normalized outputs, manifest `social` section, README + AGENTS.md docs, 41-test suite). Spec sync merged via PR #63 (`bdd79fc`): new main spec `openspec/specs/social-tables-normalization/spec.md`. Change archived as `2026-09-22-normalize-social-tables`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 41 social tests OK, 232 total game-content tests OK, change + specs strict validations PASS (15/15); staged diff reviewed (12 files, +2376 insertions, no legacy sources touched) and `git diff --check` clean.
- **Current objective:** Propose the next smallest coherent M3 change: remaining content domains (offer packs with heterogeneous/nested shapes plus a stored float artifact, globals tuning constants, inventory/taxonomy objects, images asset index, darts dynamics with patch replacement, unit-collection categories), the canonical domain model (roadmap section 8), or the Compatibility API v0 groundwork — whichever bounds smaller against the normalized package precedent.
- **Last completed change:** `normalize-social-tables` — archived as `2026-09-22-normalize-social-tables`; proposal PR #61, Apply PR #62, sync PR #63
- **Next eligible objective:** Remaining content domains (offers, globals, inventory/taxonomy, images, darts, unit collections), domain model, or Compatibility API v0 groundwork after this archive; authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: assist rewards identical and findable coins uniform 100 across all entries, carried verbatim with notes; social `description` empty in all 26 entries (quest-`hint` precedent); social ids non-sequential while findable ids run 1..10. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 15/15 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Social-tables acceptance complete (proposal PR #61, Apply PR #62, sync PR #63, archive in progress). Directly executed: build exit 0 (5/10/26), 41 tests OK, 232 total game-content tests OK, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `bdd79fc` (Merge pull request #63, social tables spec sync)
