...
- **Change status:** Proposed on branch `docs/first-render-in-godot-proposal`. M4 exit slice: minimal render-verification Godot project at `apps/client-godot/` that loads the two conversion-v1 packages read-only, resolves frame-1 placement chains, renders bounds quads with decoded SWF fill matrices and JPEG+alpha compositing, captures the viewport, and compares it programmatically against an independent source-bitmap composite with committed evidence and a non-zero-exit regression command. Artifacts: proposal, delta spec for new capability `first-render-in-godot` (7 requirements, 16 scenarios), design (9 decisions: runtime loading over resource import, bounds-quad over tessellation with the all-rectangles evidence, fill-matrix oracle, straight alpha, single-run capture/compare, display constraint, evidence paths, digest guard), tasks (5 groups, 15 tasks). `openspec validate first-render-in-godot --strict` PASS (2026-09-26). Grounding measured from the packages: every shape in both packages is straight-only (`house 4/0, lines=0`; all 28 elephant shapes 4/0), house JPEG 216×144 px equals its shape bounds, placements carry no transform fields, no live-Flash reference render exists (forbidden), so "renders correctly" is defined as authentic source-bitmap fidelity at authentic bounds/placement within a documented tolerance.
- **Current objective:** proposal stage open on `docs/first-render-in-godot-proposal`; after its PR merges, return to updated main and start Apply on a `feat/` branch using tasks 1.1–5.5. Prior context: `first-unit-conversion` archived via PR #109 (`12d3107`); M4 deliverables complete; Godot 4.7.2 installed and pinned (docs PR #110, `615c739`).
...
- **Next eligible objective:** Apply `first-render-in-godot` — implement the render-verification project per the approved task list, then Verify, Sync, and Archive the change (M4 exit assessment evidence produced inside Apply/Verify).
