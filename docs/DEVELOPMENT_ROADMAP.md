...
- **Roadmap cursor:** M3 / images normalization (stored-content coverage 20/20)
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `normalize-images` fully complete. Proposal merged via PR #81 (`e585e7f`): 607 asset paths with locale `en`, verbatim preservation, no references, no layering, asset existence explicitly M4. Apply merged via PR #82 (`9bc9f28`): implementation `61a783d` (builder/validator, image_asset schema, normalized output, manifest `images` section, README + AGENTS.md docs, 28-test suite). Spec sync merged via PR #83 (`21dea36`): new main spec `openspec/specs/images-normalization/spec.md`. Archive merged via PR #84 (`c1a9898`): change archived as `2026-09-22-normalize-images`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 28 images tests OK, 412 total game-content tests OK, change + specs strict validations PASS (20/20); staged diff reviewed (8 files, +6709 insertions, no legacy sources touched) and `git diff --check` clean.
- **Current objective:** M3 stored-content normalization is complete at 20/20 census keys (items, quests, reference tables, economy, social, taxonomy, darts, globals, offers, images — 10 builders, 17 schemas, 412 tests). Next eligible: the canonical domain model (roadmap section 8, docs/design) or Compatibility API v0 groundwork (M5) — a phase boundary needing scoping input before a new change is proposed.
- **Last completed change:** `normalize-images` — archived as `2026-09-22-normalize-images`; proposal PR #81, Apply PR #82, sync PR #83
- **Next eligible objective:** Domain model (section 8) or Compatibility API v0 groundwork; authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: all 607 locales exactly `en` (drift fails the build); extensions 470 jpg / 127 png / 10 swf with 459 leading-slash paths, all preserved verbatim; swf paths are archival references for M4, never executed or checked against disk here. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 20/20 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Images acceptance complete (proposal PR #81, Apply PR #82, sync PR #83, archive PR #84). Directly executed: build exit 0 (607 images), 28 tests OK, 412 total game-content tests OK, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `c1a9898` (Merge pull request #84, images archive)
