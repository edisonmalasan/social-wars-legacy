...
- **Roadmap cursor:** M3 / globals tuning normalization
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `normalize-globals-tuning` fully complete. Proposal merged via PR #73 (`d1a4042`): 105 loaded constants via powerup-add layering, verbatim heterogeneous values with recorded types, opaque string constants. Apply merged via PR #74 (`04b5de3`): implementation `a3a0302` (builder/validator, global_entry schema with value-type union, normalized output, manifest `globals` section, README + AGENTS.md docs, 32-test suite). Spec sync merged via PR #75 (`33114f2`): new main spec `openspec/specs/globals-tuning-normalization/spec.md`. Change archived as `2026-09-22-normalize-globals-tuning`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 32 globals tests OK, 347 total game-content tests OK, change + specs strict validations PASS (18/18); staged diff reviewed (8 files, +3056 insertions, no legacy sources touched) and `git diff --check` clean.
- **Current objective:** Propose the next smallest coherent M3 change: remaining content domains (offer packs with heterogeneous/nested shapes plus a stored float artifact, images asset index), the canonical domain model (roadmap section 8), or the Compatibility API v0 groundwork — whichever bounds smaller against the normalized package precedent.
- **Last completed change:** `normalize-globals-tuning` — archived as `2026-09-22-normalize-globals-tuning`; proposal PR #73, Apply PR #74, sync PR #75
- **Next eligible objective:** Remaining content domains (offers, images), domain model, or Compatibility API v0 groundwork after this archive; authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: 104 stored constants plus the single 6-row powerup-added schedule; value types 62 integer / 4 number / 8 string / 23 array / 8 object; 8 string constants (version lists, URL, date, depot-limits string, friend-reward CSV strings) stay opaque, never parsed. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 18/18 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Globals-tuning acceptance complete (proposal PR #73, Apply PR #74, sync PR #75, archive in progress). Directly executed: build exit 0 (104 stored / 105 loaded), 32 tests OK, 347 total game-content tests OK, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `33114f2` (Merge pull request #75, globals tuning spec sync)
