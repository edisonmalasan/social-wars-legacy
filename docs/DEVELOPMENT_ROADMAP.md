...
- **Roadmap cursor:** M3 / darts schedule normalization
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `normalize-darts-schedule` fully complete. Proposal merged via PR #69 (`3c910ab`): 27 patched entries via targets-replace layering, verbatim pools and start_date inputs, one items reference edge, served bytes out of scope. Apply merged via PR #70 (`31f3b33`): implementation `5f2f3cb` (builder/validator, darts_item schema, normalized output, manifest `darts` section, README + AGENTS.md docs, 36-test suite). Spec sync merged via PR #71 (`4f08635`): new main spec `openspec/specs/darts-schedule-normalization/spec.md`. Change archived as `2026-09-22-normalize-darts-schedule`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 36 darts tests OK, 315 total game-content tests OK, change + specs strict validations PASS (17/17); staged diff reviewed (8 files, +2273 insertions, no legacy sources touched) and `git diff --check` clean.
- **Current objective:** Propose the next smallest coherent M3 change: remaining content domains (offer packs with heterogeneous/nested shapes plus a stored float artifact, globals tuning constants, images asset index), the canonical domain model (roadmap section 8), or the Compatibility API v0 groundwork — whichever bounds smaller against the normalized package precedent.
- **Last completed change:** `normalize-darts-schedule` — archived as `2026-09-22-normalize-darts-schedule`; proposal PR #69, Apply PR #70, sync PR #71
- **Next eligible objective:** Remaining content domains (offers, globals, images), domain model, or Compatibility API v0 groundwork after this archive; authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: stored 30 darts entries are replace inputs only, 27 patched entries (ids 1..27, six-id pools) normalized with `source_layer` patched(targets); `start_date` values are derivation inputs, served dates drift via `make_dynamic` and stay unverified. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 17/17 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Darts-schedule acceptance complete (proposal PR #69, Apply PR #70, sync PR #71, archive in progress). Directly executed: build exit 0 (30 stored / 27 patched / 27 normalized), 36 tests OK, 315 total game-content tests OK, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `4f08635` (Merge pull request #71, darts schedule spec sync)
