...
- **Roadmap cursor:** M3 / economy schedules normalization
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `normalize-economy-schedules` fully complete. Proposal merged via PR #57 (`2b8d8ec`): 98 expansion / 4 town / 4 map / 50 ranking rewards, verbatim native, one items reference edge, no patch layering. Apply merged via PR #58 (`0fcc156`): implementation `b9e891d` (builder/validator, 4 schemas, 4 normalized outputs, manifest `economy` section, README + AGENTS.md docs, 44-test suite). Spec sync merged via PR #59 (`077602d`): new main spec `openspec/specs/economy-schedules-normalization/spec.md`. Change archived as `2026-09-22-normalize-economy-schedules`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 44 economy tests OK, 191 total game-content tests OK, change + specs strict validations PASS (14/14); staged diff reviewed (14 files, +4147 insertions, no legacy sources touched) and `git diff --check` clean. Prior-build outputs verified byte-identical on rebuild.
- **Current objective:** Propose the next smallest coherent M3 change: remaining content domains (offer packs with heterogeneous/nested shapes plus a stored float artifact, globals tuning constants, inventory/taxonomy, images asset index, darts dynamics with patch replacement, social tables), the canonical domain model (roadmap section 8), or the Compatibility API v0 groundwork — whichever bounds smaller against the normalized package precedent.
- **Last completed change:** `normalize-economy-schedules` — archived as `2026-09-22-normalize-economy-schedules`; proposal PR #57, Apply PR #58, sync PR #59
- **Next eligible objective:** Remaining content domains (offers, globals, inventory/taxonomy, images, darts, social), domain model, or Compatibility API v0 groundwork after this archive; authentic progressed-player saves remain unavailable and must not be synthesized.
- **Blocking issues:** None for the archived change. Source-truth observations recorded: town and map price schedules are value-identical as stored but preserved as separate schedules, never deduplicated; ranking cash is uniform 1 across all 50 levels; offer packs carry heterogeneous flat/nested item shapes with a stored float artifact (`1072.1224`) and quantity-vs-id ambiguity, so offers need a dedicated analysis change, not a bundled slice. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 14/14 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Economy-schedules acceptance complete (proposal PR #57, Apply PR #58, sync PR #59, archive in progress). Directly executed: build exit 0 (98/4/4/50), 44 tests OK, 191 total game-content tests OK, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `077602d` (Merge pull request #59, economy schedules spec sync)
