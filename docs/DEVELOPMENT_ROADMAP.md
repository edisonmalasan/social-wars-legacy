...
- **Roadmap cursor:** M4 / asset registry foundation
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `asset-registry-foundation` fully complete. Proposal merged via PR #86 (`5bc57a2`): validated on-disk registry joined against content references, observation only. Apply merged via PR #87 (`108d679`): implementation `612deba` (builder/validator, 3 schemas, registry.json + coverage.json, tool README + docs/assets/registry.md + AGENTS.md docs, 21-test suite). Spec sync merged via PR #88 (`64e84a9`): new main spec `openspec/specs/asset-registry/spec.md`. Change archived as `2026-09-22-asset-registry-foundation`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 21 registry tests OK, 412 M3 tests intact, change + specs strict validations PASS (21/21); staged diff reviewed (11 files, +27099 insertions, no assets modified, no Flash execution) and `git diff --check` clean.
- **Current objective:** M4 slice 1 done (observe/record: 3,215-file corpus, joins measured). Next smallest coherent M4 objective: SWF static inspection (header/tag inventory, symbol and embedded-asset listing without execution) to fill the registry fields parsing makes knowable — whichever bounds smaller against the registry precedent. Converted building/unit and any Godot rendering verification stay later (M5/M6 own Godot).
- **Last completed change:** `asset-registry-foundation` — archived as `2026-09-22-asset-registry-foundation`; proposal PR #86, Apply PR #87, sync PR #88
...
- **Blocking issues:** None for the archived change. Source-truth observations recorded: SWF corpus 1176 fully accounted (sprites 862, fx 205, flash 60, characters_2 26, magic 10, images 9, fonts 2, swf 1, externalized 1); item sprites 907/917 refs resolve with 10 missing names listed; magic 10/10; sounds 139/139; images basename tiers 525 single / 50 collisions / 32 missing; registry hashes are worktree bytes, distinct from baseline-blob hashes. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 21/21 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Asset-registry acceptance complete (proposal PR #86, Apply PR #87, sync PR #88, archive in progress). Directly executed: build exit 0 (3,215 files), 21 tests OK, 412 M3 tests intact, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `64e84a9` (Merge pull request #88, asset registry spec sync)
