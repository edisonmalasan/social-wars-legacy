...
- **Roadmap cursor:** M4 / sound extraction
- **Active OpenSpec change:** none
- **Lifecycle stage:** ARCHIVED
- **Change status:** `sound-extraction` fully complete. Proposal merged via PR #94 (`cdee5b6`): verbatim MP3 payload extraction with sync-offset validation, no decoding. Apply merged via PR #95 (`182e9fa`): implementation `a1a90e2` (extractor, 2 schemas, extraction.json + statuses.json, 27 converted MP3s, registry converted-dir exclusion, README + docs + AGENTS.md docs, 18-test suite). Spec sync merged via PR #96 (`eb8b523`): new main spec `openspec/specs/sound-extraction/spec.md`. Change archived as `2026-09-22-sound-extraction`. Directly executed checks under pinned Windows x64 CPython 3.9.13: clean build exit 0, 18 extraction tests OK, 60 asset-registry tests OK, 412 M3 tests intact, change + specs strict validations PASS (23/23); staged diff reviewed (40 files, +1460 insertions, no sources modified, no Flash involvement) and `git diff --check` clean.
- **Current objective:** M4 slices 1–3 done (registry, inspection, sound extraction). Next smallest coherent M4 objective: bitmap extraction (JPEG/lossless payloads to converted images) or first-conversion assembly — whichever bounds smaller against the extraction precedent. Converted building/unit and any Godot rendering verification stay later (M5/M6 own Godot).
- **Last completed change:** `sound-extraction` — archived as `2026-09-22-sound-extraction`; proposal PR #94, Apply PR #95, sync PR #96
...
- **Blocking issues:** None for the archived change. Source-truth observations recorded: all 27 sounds MP3 in `assets/swf/dynamic2.swf` with first frame sync uniformly at payload offset 9; characteristics vary (rates/mono-stereo) and are recorded, never normalized; MP3 stays MP3 with no transcoding specified; generated `assets/converted/` outputs are excluded from the registry source corpus. Non-blocking follow-ups: unused `copy_tool` helper in `tools/endpoint-catalog/test_endpoint_catalog.py`; No dedicated OpenSpec verification workflow is installed; used strict validation, independent verification, required checks, and final diff review fallback. Authentic progressed-player saves remain unavailable and unverified.
- **Last OpenSpec validation:** PASS — change strict validation and `openspec validate --specs --strict` 23/23 PASS (2026-09-22); this is not the unavailable verification workflow
- **Last implementation verification:** Sound-extraction acceptance complete (proposal PR #94, Apply PR #95, sync PR #96, archive in progress). Directly executed: build exit 0 (27 sounds), 18 tests OK, 60 asset-registry tests OK, 412 M3 tests intact, strict validations PASS under pinned CPython 3.9.13; implementation diff and containment independently reviewed.
- **Last verified commit:** `eb8b523` (Merge pull request #96, sound extraction spec sync)
